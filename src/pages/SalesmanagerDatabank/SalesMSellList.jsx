              ...
              <div key={item.id} className={styles.leadCard}>
                <div className={styles.leadInfo}>
                  <div className={styles.infoBlock}>
                    ...
                    {item.is_in_project && item.project_name && (
                      <p className={styles.inProjectTag}><strong>Involved in Project: {item.project_name}</strong></p>
                    )}
                  </div>
                  <div className={styles.infoBlock}>
                    {(item.district || item.place) && (
                      <p><strong>{[item.district, item.place].filter(Boolean).join(", ")}</strong></p>
                    )}
                    {item.address && <p><strong>{item.address}</strong></p>}
                  </div>
                  <div className={styles.infoBlock}>
                    {item.purpose && <p><strong>Purpose: {item.purpose}</strong></p>}
                    {item.mode_of_property && <p><strong>Property Type: {item.mode_of_property}</strong></p>}
                    {item.lead_category && <p><strong>Lead Category: {item.lead_category}</strong></p>}
                  </div>
