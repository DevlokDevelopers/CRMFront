...
const AdminDashboard = () => {
  ...
  const fetchCrmPerformance = async () => {
    try {
      const res = await axios.get("http://93.127.185.178:8000/databank/admin_crm_performance/", {
  ...
  const fetchLeadGraph = async () => {
    try {
      const res = await axios.get("http://93.127.185.178:8000/databank/admin_Leads_graph/", {
        headers: { Authorization: `Bearer ${accessToken}` },
      });
  ...
  const fetchUpcomingEvents = async () => {
    try {
      const res = await axios.get("http://93.127.185.178:8000/task/todays_upcoming_events/", {
        headers: { Authorization: `Bearer ${accessToken}` },
      });
      setEvents(res.data || []);
    } catch (error) {
